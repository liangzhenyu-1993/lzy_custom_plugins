...
;(function ($, window, document, undefined) {
    ...
    $.extend({
        ...
        TimeTool: {
             ...
             * @returns {*}
             */
            getIntTime: function (num, type, time) {
                num = num || 0;
                var strTime, timeArr, date = this._dealTime2Obj(time);
                ...
                if (type === "month" || type === 1) {
                    ...
                    if (num < 0) {
                        ...
                        newMon = num - month;
                        if (newMon < 0) {
                            strTime = date.getFullYear() + "" + this._dealLessTen(month - num);
                        }
                        if (newMon === 0) {
                        ...
                        if (newMon > 0) {
                            if (newMon % 12 === 0) {
                                ...
                                strTime = date.getFullYear() - parseInt(newMon / 12) - 1 + "" + this._dealLessTen(12 - newMon % 12);
                            }
                        }
                    } else {
                        newMon = date.getMonth() + 1 + num;
                        if (newMon <= 12) {
                            strTime = date.getFullYear() + "" + this._dealLessTen(newMon);
                        } else {
                            if (newMon % 12 === 0) {
                                strTime = date.getFullYear() + parseInt(newMon / 12) - 1 + "12";
                            } else {
                                strTime = date.getFullYear() + parseInt(newMon / 12) + "" + this._dealLessTen(newMon % 12);
                            }
                        }
                    }
                }
                if (!type || type === "day" || type === 2) {
                    ...
                    strTime = timeArr[0] + this._dealLessTen(timeArr[1]) + this._dealLessTen(timeArr[2]);
                }
                if (type === "hour" || type === 3) {
                    date.setTime(date.getTime() + num * 60 * 60 * 1000);
                    timeArr = this._getDateArr(date);
                    strTime = timeArr[0] + this._dealLessTen(timeArr[1]) + this._dealLessTen(timeArr[2]) + this._dealLessTen(timeArr[3]);
                }
                if (type === "min" || type === 4) {
                    date.setTime(date.getTime() + num * 60 * 1000);
                    timeArr = this._getDateArr(date);
                    strTime = timeArr[0] + this._dealLessTen(timeArr[1]) + this._dealLessTen(timeArr[2]) + this._dealLessTen(timeArr[3]) + this._dealLessTen(timeArr[4]);
                }
             ...
             * @returns {*}
             */
            getFormatTime: function (num, type, time, str1, str2) {
                return this.formatTime(this.getIntTime(num, type, time), str1, str2);
            },
            getIntMonth: function (num, time) {
                return this.getIntTime(num, "month", time);
            },
            getIntDay: function (num, time) {
                return this.getIntTime(num, "day", time);
            },
            getIntHour: function (num, time) {
                return this.getIntTime(num, "hour", time);
            },
            getIntMin: function (num, time) {
                return this.getIntTime(num, "min", time);
            },
            getFormatMonth: function (num, time, str1, str2) {
                return this.formatTime(this.getIntTime(num, "month", time), str1, str2);
            },
            getFormatDay: function (num, time) {
                return this.formatTime(this.getIntTime(num, "day", time), str1, str2);
            },
            getFormatHour: function (num, time) {
                return this.formatTime(this.getIntTime(num, "hour", time), str1, str2);
            },
            getFormatMin: function (num, time) {
                return this.formatTime(this.getIntTime(num, "min", time), str1, str2);
            },
            getIntTimeBefore: function (num, type, time) {
                return this.getIntTime(-(num || 0), type, time);
            },
            getFormatTimeBefore: function (num, type, time, str1, str2) {
                return this.formatTime(this.getIntTime(-(num || 0), type, time), str1, str2);
            },
            getIntMonthBefore: function (num, time) {
                return this.getIntTime(-(num || 0), "month", time);
            },
            getIntDayBefore: function (num, time) {
                return this.getIntTime(-(num || 0), "day", time);
            },
            getIntHourBefore: function (num, time) {
                return this.getIntTime(-(num || 0), "hour", time);
            },
            getIntMinBefore: function (num, time) {
                return this.getIntTime(-(num || 0), "min", time);
            },
            getFormatMonthBefore: function (num, time, str1, str2) {
                return this.formatTime(this.getIntTime(-(num || 0), "month", time), str1, str2);
            },
            getFormatDayBefore: function (num, time) {
                return this.formatTime(this.getIntTime(-(num || 0), "day", time), str1, str2);
            },
            getFormatHourBefore: function (num, time) {
                return this.formatTime(this.getIntTime(-(num || 0), "hour", time), str1, str2);
            },
            getFormatMinBefore: function (num, time) {
                return this.formatTime(this.getIntTime(-(num || 0), "min", time), str1, str2);
            },
            getIntTimeAfter: function (num, type, time) {
                return this.getIntTime((num || 0), type, time);
            },
            getFormatTimeAfter: function (num, type, time, str1, str2) {
                return this.formatTime(this.getIntTime((num || 0), type, time), str1, str2);
            },
            getIntMonthAfter: function (num, time) {
                return this.getIntTime((num || 0), "month", time);
            },
            getIntDayAfter: function (num, time) {
                return this.getIntTime((num || 0), "day", time);
            },
            getIntHourAfter: function (num, time) {
                return this.getIntTime((num || 0), "hour", time);
            },
            getIntMinAfter: function (num, time) {
                return this.getIntTime((num || 0), "min", time);
            },
            getFormatMonthAfter: function (num, time, str1, str2) {
                return this.formatTime(this.getIntTime((num || 0), "month", time), str1, str2);
            },
            getFormatDayAfter: function (num, time) {
                return this.formatTime(this.getIntTime((num || 0), "day", time), str1, str2);
            },
            getFormatHourAfter: function (num, time) {
                return this.formatTime(this.getIntTime((num || 0), "hour", time), str1, str2);
            },
            getFormatMinAfter: function (num, time) {
                return this.formatTime(this.getIntTime((num || 0), "min", time), str1, str2);
            },
            /**
             * 格式化时间
